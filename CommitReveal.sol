...
contract CommitReveal {
  ...
  struct Commit { // when returning, it returns according to the struct
    // to type cast => look up the ascii table and hexadecimal, this is also a way to hash the st 
    // unsigned256 bit = bytes32 (which means st <= 32 bytes) used when we want to type cast
    bytes32 commit; // a mapping between what's hashed and what we commit to blockchain
    uint64 block;
    bool revealed;
  }
  ...
  function commit(bytes32 dataHash) public { 
    ...
    emit CommitHash(msg.sender,commits[msg.sender].commit,commits[msg.sender].block); // sent event for people to see what's goin on
    // (who commited, what's committed, which block is it commited on)
  }
